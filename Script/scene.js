...
Scene.save = function () {
  if (this.state === 'open') {
    ...
    scene.contrast = this.contrast
    scene.ambient = this.ambient
    scene.terrains = this.terrains
...
// 加载预设对象ID
Scene.loadPresets = function IIFE() {
  const load = nodes => {
    const {presets} = Scene
    for (const node of nodes) {
      const {presetId} = node
      if (presetId !== undefined) {
        presets[presetId] = node
      } else {
        load(node.children)
      }
    }
  ...
  if (this.showAnimation) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (emitter !== undefined) {
        const point = this.getConvertedCoords(particle)
        emitter.startX = point.x
        emitter.startY = point.y
    ...
    for (let i = 0; i < li; i++) {
      ...
      do {
        ...
        if (code < 0x10000) {
          ...
          actors[code & 0x0ffff].player.draw(animAlpha)
        } else {
          animations[code & 0x0ffff].player.draw(animAlpha)
        }
      } while ((si = set[si + 1]) !== 0)
    }
    gl.batchRenderer.draw()
  ...
  if (this.showLight) {
    ...
    if (count !== 0) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        light.instance.draw(projMatrix)
      }
      gl.blend = 'normal'
    }
    gl.resetViewport()
...
Scene.drawParticleEmitterWireframe = function () {
  if (this.target?.class === 'particle') {
      ...
      for (const {area} of emitter.data.layers) {
        switch (area.type) {
          ...
          case 'rectangle': {
            ...
            const ar = ax + aw / 2
            const ab = ay + ah / 2
            this.drawRectWireframe(al, at, ar, ab, ax, ay, angle)
            this.drawTargetAnchor(particle, angle)
            break
          }
          case 'circle': {
            ...
            this.drawOvalWireframe(ax, ay, ar, ar, 0xffffffff)
            this.drawTargetAnchor(particle, angle)
            break
          }
        }
    ...
    this.drawTargetAnchor(particle, 0)
  }
}
...
Scene.selectParticleEmitter = function (x, y) {
  ...
  const mx = point.x
  const my = point.y
  const twh = this.tileWidth / 2
  const thh = this.tileHeight / 2
  const particles = this.particles
  const length = particles.length
  for (let i = length - 1; i >= 0; i--) {
    const particle = particles[i]
...
Scene.switchTerrain = function () {
  ...
  context.terrain = (context.terrain + 1) % 3
...
Scene.planToSaveTerrains = function () {
  this.context.scene.terrainsChanged = true
}
