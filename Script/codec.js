...
Codec.encodeScene = function (scene) {
  ...
  for (const [key, value] of Object.entries(scene)) {
    switch (key) {
      case 'terrains':
        if (scene.terrainsChanged) {
          scene.terrainsChanged = false
          scene.terrainsCode = this.encodeTerrains(value)
        }
        SCENE.terrains = scene.terrainsCode
        break
      default:
        SCENE[key] = value
        break
...
// 解码场景
Codec.decodeScene = function (code) {
  const SCENE = JSON.parse(code)
  Object.defineProperty(SCENE, 'terrainsCode', {writable: true, value: SCENE.terrains})
  Object.defineProperty(SCENE, 'terrainsChanged', {writable: true, value: false})
  SCENE.terrains = this.decodeTerrains(SCENE.terrains, SCENE.width, SCENE.height)
  const tilemaps = this.getTilemaps(SCENE)
  for (const tilemap of tilemaps) {
...
// 解码队伍关系
Codec.decodeRelations = function (code, length) {
  const BYTES = this.textEncoder.encode(code)
  const BYTES_LENGTH = BYTES.length
...
// 编码克隆数据
Codec.encodeClone = function IIFE() {
  const {ceil, log2} = Math
  return (array, index, count) => {
    const bits = ceil(log2(count + 1))
    const bytes = ceil(bits / 5)
