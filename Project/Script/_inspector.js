...

{
...

	{
		...
		UIText.initialize = function () {
			...
			$('#uiText-lineSpacing-slider').synchronize(
				$('#uiText-lineSpacing')
			)
...

	{
		...
		UITextBox.initialize = function () {
			...
			// 侦听事件
			const elements =
				$(`#uiTextBox-type, #uiTextBox-align, #uiTextBox-text,
...

	{
		...
		UIReference.initialize = function () {
			// 侦听事件
			const elements = $(
				'#uiReference-prefabId, #uiReference-synchronous'
			)
			elements.on('input', this.paramInput)
			elements.on('focus', Inspector.inputFocus)
			elements.on('blur', Inspector.inputBlur(this, UI))
