...
const Workshop = new (class WorkshopManager {
	...
	async updateItem(id) {
		await this.workshop.updateItem(id, {
			...
			previewPath: File.route(
				'Assets/角色/头像/Chest-1.d095eb446178fd84.png'
			),
			contentPath: File.route(
				'Assets/角色/头像/Chest-1.d095eb446178fd84.png'
			),
			tags: ['tag1', 'tag2'],
	...
	async getUserItems() {
		...
		const result = await workshop.getUserItems(
			...
			workshop.UserListOrder.TitleAsc,
			{ creator: APP_ID, consumer: APP_ID }
		)
		return result.items
	}
	...
	async getAllItems() {
		...
		const result = await workshop.getAllItems(
			1,
			workshop.UGCQueryType.RankedByVote,
			workshop.UGCType.Items,
			APP_ID,
			APP_ID,
...
const WorkshopItem = new (class WorkshopItemManager {
	...
	initialize() {
		...
		$('#workshop-item-type')
			.enableHiddenMode()
			.relate([
				{ case: 'application', targets: [$('#workshop-item-content')] }
			])
	}
